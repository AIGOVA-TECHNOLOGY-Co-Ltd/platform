...
<form method="GET" action="{{ route('user.role.index') }}" class="sm:flex sm:space-x-4">
    <div class="flex-grow mt-2 sm:mt-0">
        <input type="search" name="search" class="form-control form-control-lg"
            placeholder="{{ __('role-index.Search...') }}" value="{{ $search }}" data-table-search="#role-list-table">
    </div>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    ...
                    <form action="{{ route('user.role.destroy', $role['id']) }}" method="POST" style="display:inline;"
                        onsubmit="return confirm('{{ addslashes(__('role-delete.confirm')) }}');">
                        @csrf
                        @method('DELETE')
